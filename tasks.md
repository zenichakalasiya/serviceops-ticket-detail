...
## 68. My Assets and My CIs — stacked, filled, and re-ranked
- **Status:** done
- **Where:** `SupportPortalPreview.tsx`
...
- **⚠️ The FILL is on the TILES, not the card.** Tinting the card and leaving its tiles white was built first and reversed on request: it made these two the only cards on the page constructed differently from the four around them, so the page had two card languages and the difference read as a state rather than a kind. Every card is white with a hairline; the separation the tiles need comes from filling the tiles, which keeps one card treatment for everything. The tiles carry no border of their own — four hairline rectangles inside a hairline rectangle is the box-inside-a-box the tinted card was avoiding, arrived at from the other direction. The icon badge went white as the tile took the fill, so it stays a step away from whatever it sits on.
...
- **The tile fill is `#F9FAFB`,** the value you specified. Hover steps to `#F3F4F6` rather than the old `#EEF2F7`: the new fill is a neutral grey where the first one was blue-tinted, so the previous hover would have shifted the tile's HUE rather than just its value — and a colour change reads as a different KIND of state change. `#F3F4F6` is the neutral this product already hovers to, on the close button across all 26 popups.
- **Verified:** requests and approvals stay side by side at 261px each; **My Assets is 538px at y=1086 and My CIs 538px at y=1323** — full width, stacked. The cards are `rgb(255,255,255)` with the same `0.888889px #E5E7EB` hairline as My Open Requests; the tiles are `rgb(249,250,251)` with `border: 0px` at 247px, badge white, 2 per row. Card height went **401 → 221px**, so the space under the last tile is 27px of padding rather than 200px of nothing. A tile reads "Dell Latitude 5440 / AST-3 · Laptop", with all four names in full.
...
## 72. The board had been dropping most of what was written on it
- **Status:** done
- **Where:** `scripts/build-tasks.mjs` · `tasks.md`
- **You asked:** add this session's work to the task board.
- Tasks 69–71 were already on it, but adding them turned up something worse: **the board had been publishing about a third of each entry.** The parser only ever matched `- **Key:** value` — a bold label ending in a COLON — so every `- **⚠️ …**` bullet was discarded in silence. Those bullets are where the reasoning for a change lives, so 59 of 94 tasks were showing their Where / You asked / Verified rows and none of the thinking between them. 271 lines had been written into a file that never rendered them, with nothing anywhere saying so.
- Non-keyed bullets are captured as **notes** now and render under the field rows, inside the same Detail disclosure. Wrapped continuation lines attach to whichever of the two came last.
- **⚠️ And they arrived as cards inside a card.** The task-card style was a BARE `li`, so it painted every list item on the page. `#parked li` had already had to override background, border, radius and padding to escape it — which was the warning that the selector was too broad, a warning nothing acted on until a second nested list hit it. Scoped to `#list > li`, which is the only thing it was ever describing.
- **⚠️ Entry 68 was describing a state that no longer existed.** Its title still said "tinted" and its Verified line still read `rgb(245,247,250)` cards with white tiles — the FIRST build, which you reversed twice after it. A record that says the opposite of what shipped is worse than no record. Retitled, and the verified line now measures the white card with `#F9FAFB` tiles that is actually on screen.
...
## 86. Template 1 — Search Spotlight, and two bugs it exposed in the hero
- **Status:** done
- **Where:** `SUPPORT-PORTAL-TEMPLATE-BRIEF.md` (new) · `supportPortalData.ts` · `SupportPortalBuilder.tsx` · `SupportPortalPreview.tsx` · `portalStyleResolver.ts`
- **You asked:** prove the design sense — one template, new styling, the same data.
- **The reference board is a MOOD BOARD, not a kit.** 45 pasted screenshots (Apple, Zendesk, Discord, Airtable…), every one a raster image: no components, no auto-layout, no variables. There is nothing in it to build from, only to look at.
- **A template is a PRESET, not a renderer.** The page was already config all the way down — band order, row order, column counts, the hero's height, colour, alignment and search width. So `PortalTemplate` gained a `seed`, and the builder starts from it. That is this codebase's own rule, stated on `BLOCK_ORDER_V2`: *"Seeds, not a second renderer."* ⚠️ It also means the other four templates are now data, not code.
- **⚠️ `rowOrder` is MERGED over the defaults, never replaced.** A seed names only the rows it rearranges, and `rowOf` still consults `records` even when that band is off the page — a seed that replaced the map wholesale would leave those cards unable to move, with nothing on screen saying why.
- **Search Spotlight:** flat indigo band (not the default gradient, which competes with the field sitting on it), 340px, white 30px question, 52% pill search. **Most Read Knowledge moves to the first card position, ahead of My Open Requests** — that one reorder is the whole thesis. Favourite Services, Most Used Services, My Assets and My CIs come off the page and stay in the palette.
- **⚠️ TWO PRE-EXISTING BUGS, both on the shipping default portal, both found by measuring rather than looking.** (1) `roleStyle` always emitted `fontSize` from `ROLE_BASE_PX`, which overrode the element's own class — **the hero heading has been rendering at 16px on every portal, not the 30px it is designed at**. It now emits a size only when a human chose one, the same rule the file already applies to `color` and `textAlign` two lines below. (2) `roleStyle` returns an explicit `color: undefined` while the colour is still the theme's, and it was spread AFTER the hero's own `headingColor` — **a spread `undefined` DELETES the key it lands on**, so the banner's heading colour was thrown away and the text fell back to near-black. Same trap `fillCss` already carries a warning about.
- **⚠️ Measured contrast was 1.72:1** against the 4.5 this product's own meter demands. Now 30px, `#FFFFFF`, ~9.9:1 on the indigo.
- **⚠️ The brief was WRONG about one thing and says so.** It called for dropping the action cards' 62px overlap into the banner. Built and looked at, the cards anchor the bottom of a band that would otherwise end on nothing. Recorded as a reversal in the brief rather than quietly deleted.
- **Verified:** bands render `hero · quick · work` and nothing else; hero paints `rgb(30,58,138)`; heading 30px `rgb(255,255,255)`; the work row's VISUAL order is Most Read (x=105) → My Open Requests (487) → Pending Approvals (869), placed by CSS `order` 0/1/2. Every record on the page is the same ServiceOps data. Build green; 333 type errors before the change, 333 after.

## 87. Spotlight, restyled — a banner, a card treatment and a search that moved
- **Status:** done
- **Where:** `SupportPortalPreview.tsx` · `supportPortalData.ts`
- **You said:** it looks the same as the current portal. **You were right, and it was the fair criticism** — the first pass changed a colour and an order, which is a variant, not a template. Three keys now change SHAPE rather than hue, and they are config, so a template stays a preset.
- **`hero.bannerStyle: 'gradient'`** — the band deepens from its own colour into `#0B1B3F` with a soft radial highlight off the top-left. A large flat rectangle of one colour reads as printed; the falloff gives it a light source, which is what makes the white field resting on it look lit rather than pasted. ⚠️ Same "Colour" tab, one key along — a separate tab would have made the admin choose a KIND before choosing the colour they came to set.
- **`hero.searchPlacement: 'floating'`** — the search leaves the banner and lands ACROSS its bottom edge, half on the colour and half on the page. In the default portal the field is furniture inside a picture; here it is the page's own control resting on one. ⚠️ **Moved, not duplicated** — same `Sel`, same id, same config, because two search bars in the tree would be two things to keep in step and one would eventually be edited while the other showed.
- **`page.cardLook: 'spine'`** — cards drop the hairline box for a borderless panel at 14px with a soft shadow and a **coloured left edge**: blue is your work, amber is waiting on you, green is something to read. ⚠️ Keyed by card ID, never by position, so the colour names a KIND and survives a reorder. ⚠️ The hues are the product's own status colours, not new ones — a fresh palette would have been a second colour language on a page that already has one.
- **⚠️ THE BANNER IS SHORTER, NOT TALLER.** The first pass took it to 340px and centred the text, putting a third of the first viewport into empty colour — the banner became the biggest thing on a page whose subject is a text field. 210px, and it is a backdrop.
- **⚠️ A negative margin was NOT enough for the floating search.** In the flow it sat above the band's 86px of reserved bottom padding — measured **96px clear of the edge**, which is not straddling anything, it is a search bar low in a banner. It is absolute against the bottom edge and pulled down by half its own height, so the effect no longer depends on what padding the band happens to carry. That also needs `overflow-visible` on the band, and the decorative artwork therefore got its own clip.
- **⚠️ I committed the exact fault this file warns about, and had quoted.** The action cards were left on the hairline treatment while the record cards went borderless — **two card languages one band apart**, which reads as a state rather than a kind. They follow `cardLook` now. **No spine on them, though:** a spine names a kind of record, an action card is a destination, and four hues across four destinations would invent a taxonomy the product does not have.
...
## 89. Only real templates in the gallery, and a blank page that says so
- **Status:** done
- **Where:** `supportPortalData.ts` · `SupportPortalTemplateGallery.tsx` · `CreateSupportPortalModal.tsx` · `AdminSupportPortalModule.tsx` · `SupportPortalBuilder.tsx` · `SupportPortalPreview.tsx`
- **You asked:** drop the templates that are only a banner colour; make the empty portal fill the screen; and stop the palette greying out widgets a blank page does not have.
- **Six templates withheld, not deleted** — Classic Service Desk, Service Catalog First, Knowledge Hub, People & HR Desk, Minimal Landing, Announcements & Status. Each has no `seed`, so it produced the default arrangement under a different name with a recoloured wireframe: **six tiles that all open the same page**, which is a promise the gallery cannot keep. `hidden` follows the same rule `PortalElement.hidden` does — names, categories, accents and block lists all stay, and each comes back on one word once it has a design.
- **⚠️ ONE derived list, `VISIBLE_TEMPLATES()`**, because three surfaces show templates — the gallery, the create dialog and the "Browse N templates" count — and filtering at each call site is exactly how a withheld template stays visible in one of them. The create dialog already had its own copy of the filter. ⚠️ `PORTAL_TEMPLATES` stays the FULL list: `accentFor` resolves a page's `source` through it, and a portal built from a template that has since been withheld must keep its accent.
- **⚠️ The gallery opened on a tile that was not there.** `useState(PORTAL_TEMPLATES[0].id)` — the first entry is now withheld, so the detail rail described a template nobody could see and the confirm button would have used it.
- **⚠️ THE PALETTE BUG WAS A STATE/RENDER DISAGREEMENT.** `blockOrder`, `rowOrder` and `content.quick` are seeded with the standard arrangement whatever the page's start was — the preview just does not render them when `blank`. So the state said "My Open Requests is on the page" while the canvas showed an empty portal, and **every predefined row was greyed with a tick on a page carrying nothing**. What a blank page has is whatever has since been dropped into it.
- **⚠️ `h-full` could not resolve, and a min-height would have been a guess.** The canvas card had auto height inside a plain block scroller, so the page root's `min-h-full` had nothing to measure against. The scroller is a flex column and the card is `flex-1` — it fills when the page is short and still grows when the page is long, which is one rule instead of a height plus an override.
...
## 91. Twelve portal directions, designed rather than wireframed
- **Status:** done
- **Where:** [docs/portal-directions.html](docs/portal-directions.html) · [docs/combination-grid.md](docs/combination-grid.md)
- **You gave me:** "can we have UI design instead of wireframe, so that i can compare the UI and layouts?" — with per-domain variety in the banner and the card treatment.
- **The method is what makes these twelve different from twelve.** Layouts 1–6 in one context condition layout 7, and the set converges on a hero + an action row + a card grid however hard you push. So: a combination grid FIRST (six axes, one row per layout, no two rows sharing more than two values), then **twelve isolated agents**, each given only its own row and blind to the other eleven.
- **⚠️ Four agents, in total isolation, independently named their layout "Front Desk."** The layouts diverge; the naming space converges. They are disambiguated here as Front Desk · Reception · Console · Lobby.
- **Four rows were built from a BAN, not a choice** — no banner (Switchboard) · no grid of equal cards (Thread) · inverted order, data before the door (Desk Before Door) · half the vertical space, ~378px (Service Counter). A ban is the only reliable way to find out whether an element was ever load-bearing.
- **These are UI, not wireframes:** one component vocabulary and twelve token blocks, so a direction is judged on its own palette, radius, density, card anatomy and banner. **Every banner treatment is pure CSS** — no image assets, so the sheet is one file.
- **Coverage:** A 12/18 · C 10/10 · D 9/9 · E 8/8 · F 9/9; every B sub-variable 100%. Exactly one pair reaches the two-shared-axis ceiling (Threshold ~ Service Counter, on `C3` and `D1`).
- **⚠️ Five defects were found by MEASURING the page, not by looking at it.** A DOM sweep for cards whose content ends more than 45px above their own bottom edge found **19 across 11 directions** — a card that stretches to fill a column while carrying three rows reads as an empty state, which is the one thing every direction has to be able to disprove. Fixed at each card’s own row pitch; re-measured to zero.
- **⚠️ The first sweep missed every card carrying an extra class** (`<div class="c sp">`, Front Desk’s accent stripe) because the matcher tested `class="c"` with the closing quote. And Thread’s feed rows are `.fd`, not `.l` — topping it up with `.l` spans at `gap:0` produced a solid pink slab. Two passes, not one.
- **⚠️ Threshold’s search and action row were two objects straddling one edge.** They are now ONE dock whose internal seam IS the banner’s bottom edge — the edge is crossed exactly once, by one object.
- Atrium’s largest bento tile held only a search field, so the treatment that states priority by SIZE was stating it with an empty box. Focus Desk’s illustration circles sat under the headline and through the search (an `::after` with no `z-index` paints over static content). The Command Rail’s rail was 34px of unlabelled squares — an icon strip, not the labelled rail the whole direction rests on.
- **Verified in the browser:** 12 direction sections + 2 reference sections render; **0 cards with excess slack** (was 19); **0px horizontal overflow** at 1500px; the dock measures 490×99 straddling its banner by exactly 28px; dark mode resolves (`body` #0E131A, ink #C4D0DD) with the twelve miniatures deliberately holding their own light identity — a portal design does not change because the reviewer’s OS is dark.
- **Unexplored, and what a second round would cover:** A2 linear gradient · A6 isometric · A8 3D logo forms · A11 photo + shape mask · A13 glassmorphic · **A17 live data motif** — the notable omission being a banner whose artwork IS the tenant’s live open-ticket count.
